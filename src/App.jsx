...
export default function GrandBrandMediaSite() {
  const slogans = useMemo(
    () => ["Launch louder.", "Look cinematic.", "Rank locally.", "Sell on autopilot."],
    []
  );
  ...
  const heroImg =
    "https://images.unsplash.com/photo-1496302662116-85c6d2a7f49a?q=80&w=1920&auto=format&fit=crop";
    ...
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28 sm:py-36">
          ...
          <motion.h1
           ...
           className="mt-5 text-4xl sm:text-6xl font-black leading-[1.15] pb-2"
          >
            Grand Brand Media
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-300 via-cyan-200 to-emerald-300">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <a href="#pricing" className="rounded-2xl bg-white text-neutral-900 font-semibold px-5 py-3 shadow hover:shadow-lg inline-flex items-center gap-2">
              See Plans <ArrowRight className="h-4 w-4" />
            </a>
            <a href="#contact" className="rounded-2xl bg-fuchsia-600/30 border border-fuchsia-300/30 px-5 py-3 font-semibold backdrop-blur hover:bg-fuchsia-600/40 inline-flex items-center gap-2">
              Talk to Us <Phone className="h-4 w-4" />
            </a>
            <span className="text-white/80 inline-flex items-center gap-2 text-sm">
              <Star className="h-4 w-4" /> No contracts • Cancel anytime
            </span>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {gridImgs.map((src, idx) => (
              <motion.div
                ...
                className="aspect-[4/5] rounded-3xl overflow-hidden border border-white/10 shadow-lg"
              >
      ...
      <section id="pricing" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <p className="mt-6 text-center text-xs text-neutral-400">
          Prices are month-to-month. Ask about annual discounts and add-ons. No long-term contracts.
        </p>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-white">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-3">
              <a href="#contact" className="rounded-2xl bg-white text-neutral-900 font-semibold px-5 py-3 shadow hover:shadow-lg">
                Book a Free Consult
              </a>
              <a href="#pricing" className="rounded-2xl border border-white/60 text-white px-5 py-3 inline-flex items-center gap-2 hover:bg-white/10">
          ...
          <div>
            ...
            <ul className="mt-6 space-y-3 text-neutral-200">
              <li className="flex items-center gap-2">
                <Phone className="h-5 w-5 text-fuchsia-300" /> (302) 487-0516
              </li>
              <li className="flex items-center gap-2">
                <Mail className="h-5 w-5 text-fuchsia-300" /> support@grandbrandmedia.com
              </li>
              <li className="flex items-center gap-2">
                <CheckCircle2 className="h-5 w-5 text-fuchsia-300" /> Mon–Fri • 9am–6pm
              </li>
            </ul>
          </div>

          <form
            action="https://formspree.io/f/mvzdddgv"
            method="POST"
            className="bg-neutral-900/60 border border-white/10 rounded-2xl p-6 shadow"
          >
            ...
            <button
              type="submit"
              className="mt-4 w-full rounded-2xl bg-fuchsia-500 text-white font-semibold px-5 py-3 hover:bg-fuchsia-600"
            >
              Send
            </button>
  ...
  return (
    <header className="sticky top-0 z-50 bg-neutral-950/70 backdrop-blur border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <a href="#contact" className="hidden md:inline-flex items-center gap-2 rounded-xl bg-white text-neutral-900 px-3 py-2 text-sm font-semibold shadow hover:shadow-lg">
...
function AnimatedPill({ text }) {
  return (
    <motion.div
      ...
      className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-4 py-2 text-sm font-semibold"
    >
  ...
  return (
    <div className="rounded-2xl border border-white/10 p-5 shadow-sm bg-neutral-900/50">
      <div className="flex items-start gap-3">
        <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-fuchsia-500 to-cyan-400 text-white grid place-items-center shadow-sm">
          {icon}
        </div>
          ...
          <ul className="mt-3 space-y-2 text-sm text-neutral-300">
            {points.map((p, idx) => (
              <li key={idx} className="flex items-start gap-2">
                <CheckCircle2 className="h-4 w-4 text-fuchsia-300 mt-0.5" />
                <span>{p}</span>
              </li>
            ))}
          </ul>
  ...
  return (
    <div className={`rounded-3xl p-6 border ${highlight ? "bg-gradient-to-b from-fuchsia-900/30 to-cyan-900/20 border-white/20" : "bg-neutral-900/60 border-white/10"} shadow-xl`}>
      <div className="flex items-center justify-between">
        ...
        {highlight && <span className="text-xs rounded-full bg-white/15 px-2 py-1 uppercase tracking-wide">Most Popular</span>}
      </div>
      ...
      <a
        href="#contact"
        className={`mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 font-semibold ${
          highlight ? "bg-white text-neutral-900 hover:shadow-lg" : "bg-fuchsia-500 hover:bg-fuchsia-600 text-white"
        }`}
      >
        {cta}
      </a>
...
function Input({ label, ...props }) {
  return (
    <div>
      ...
      <input
        {...props}
        className="mt-1 w-full rounded-xl border border-white/10 bg-neutral-900/70 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-fuchsia-500"
      />
    </div>
  );
}
